...
export const isDateString = (val: any): boolean => isStringFull(val) &&
...
export const checkService = (_this: any) => {
    if (!_this.service) {
        throw new Error('Service is not defined, Make Sure Controller is must have defined property name with service example: service: UserService');
    }
};


export const normalizeColumnName = (columnPath: string, quote: string = '"') => {
    // Split the path into parts, handling both dot notation and quoted parts
    const parts = columnPath.split('.');

    // Process each part to ensure proper quoting
    const normalizedParts = parts.map(part => {
        // Remove any existing quotes
        const cleanPart = part.replace(new RegExp(quote, 'g'), '');
        // Add quotes around the part
        return `${quote}${cleanPart}${quote}`;
    });

    // Join the parts with dots and wrap the alias
