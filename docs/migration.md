...
- **Hidden / sensitive fields.** `@CrudHidden()` on a column/relation (or
  ...
  `hiddenFields` option is now enforced. See [Hiding sensitive fields](./querying#hiding-sensitive-fields).
...
- **Large queries via method override.** Send a too-long filter in a POST body,
  handled as a GET. See [Large queries](./large-queries).
...
[What the write hooks receive](./lifecycle-hooks#what-the-write-hooks-receive).
...
version. See the [CHANGELOG](https://github.com/ack-solutions/nest-crud/blob/main/CHANGELOG.md)
